...
export default class PessoaColecao {
    ...
    async salvar(pessoa: Pessoa) {
        ...
        if (pessoa?.id) {
            const pesRef = await doc(this.colecao(), "pessoa")
            const docRef = collection(pesRef, pessoa.id)
            const ddRef = await doc(docRef, pessoa.id)
            console.log("alterou")
            await updateDoc(ddRef, this.#conversor.toFirestore(pessoa))
